...
namespace Downplay.Origami.Placement
{
    ...
    public class ShapePlacementParsingStrategy : IShapeTableProvider {
        ...
        private readonly IEnumerable<IPlacementPredicateProvider> _predicateBuilders;
        private readonly IPlacementAlterationProvider _alterationBuilder;
        ...
        private void ProcessPlacementFile(ShapeTableBuilder builder, FeatureDescriptor featureDescriptor, PlacementFile placementFile) {
            ...
            foreach (var entry in entries) {
                ...
                var segments = shapeLocation.Location.Split(';').Select(s => s.Trim()).Where(s=>!String.IsNullOrWhiteSpace(s));
                    ...
                    if (!segment.Contains('=')) {
                        ...
                        switch (property) {
                            ...
                            case "wrapper":
                                ...
                                break;
                            default:
                                // Unknown properties use alteration builders
                                _alterationBuilder.Alter(placement, property, value);
                                break;
                        }
                    }
                }
        ...
        protected Func<ShapePlacementContext, bool> BuildPredicate(Func<ShapePlacementContext, bool> predicate, KeyValuePair<string, string> term) {
            ...
            switch (term.Key) {
                ...
                default:
                    Func<ShapePlacementContext, bool> final = predicate;
                    foreach (var builder in _predicateBuilders) {
                        final = builder.Predicate(term,final);
                    }
                    return final;
            }
        }
