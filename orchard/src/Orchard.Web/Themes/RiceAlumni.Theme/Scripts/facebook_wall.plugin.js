(function($) {
	$.fn.facebook_wall = function(options) {
		...
		$.getJSON(graphPOSTS, function(posts) {
					...
					output += '<li class="post' + post_class + 'avatar-size-' + options.avatar_size + '">';
						...
						if (this.type == 'link' || this.type == 'photo' || this.type == 'video') {
							...
							output += '<div class="media' + media_class + ' clearfix">';
								if (this.picture != null || this.picture != undefined) {
									output += '<div class="image"><a target="_blank" href="' + this.link + '"><img src="' + this.picture + '" /></a></div>';
								} else if (this.object_id != null || this.object_id != undefined) {
									output += '<div class="image"><a target="_blank" href="' + this.link + '"><img src="' + (graphURL + this.object_id + '/picture?type=large') + '" /></a></div>';
								}
						...
						output += '<div class="meta-footer">';
							...
							if (this.likes != null || this.likes != undefined) {
								output += '<span class="seperator">&middot;</span><span class="likes">' + this.likes.count + ' Likes</span>';
							} else {
								output += '<span class="seperator">&middot;</span><span class="likes">0 Likes</span>';
							}
							if (this.comments.count == 1) {
								output += '<span class="seperator">&middot;</span><span class="comments">' + this.comments.count + ' Comment</span>';
							} else {
								output += '<span class="seperator">&middot;</span><span class="comments">' + this.comments.count + ' Comments</span>';
							}
							split_id = this.id.split('_');
							output += '<div class="actionlinks"><span class="like"><a href="http://www.facebook.com/permalink.php?story_fbid=' + split_id[1] + '&id=' + split_id[0] + '" target="_blank">Like This!</a></span><span class="seperator">&middot;</span><span class="comment"><a href="http://www.facebook.com/permalink.php?story_fbid=' + split_id[1] + '&id=' + split_id[0] + '" target="_blank">Leave a Comment</a></span></div>';
						output += '</div>';
						...
						if (this.comments.count > 0 && this.comments.data != undefined) {
							output += '<ul class="comment-list">';
								...
								if (this.comments.data.length < this.comments.count) {
									output += '<li class="read_more"><a href="http://www.facebook.com/permalink.php?story_fbid=' + split_id[1] + '&id=' + split_id[0] + '" target="_blank">See all comments &raquo;</a></li>';
								}
							output += '</ul>';
						}
			...
			// Deferred - http://wordpress.tv/2012/08/27/andy-peatling-mistakes-i-made-using-jquery-and-how-to-avoid-them/ - #10
		...
		function timeToHuman(time) {
			...
			if (time_difference < 10) {
				...
				var months = new Array('Janurary', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');
