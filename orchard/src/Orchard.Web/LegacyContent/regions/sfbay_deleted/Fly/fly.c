...
**  Copyright (c), The University of Melbourne, 1994-1999
**  Created: 9 December 1994
**  Last Update: 15th January 1999
**
**  Uses the gd library by Thomas Boutell, boutell@boutell.com
**  gd: Copyright 1994-1998, Quest Protein Database Centre, Cold Spring Harbour Labs
**
**  Contributions from:
...
FILE *get_file(int *line_number, char **filename)
{
	...
	if (fname[0]=='`') {
		...
		*filename = (char *) my_newmem( n );
		...
		in = fdopen(p[0], "rb");
#else
	fprintf(stderr,"Error: Reading GIF from external command not supported on this Operating System\n");
	exit(1);
#endif
	} else {
